//
//  CustomDynamicProgressView.swift
//  Widgets
//
//  Created by Webcash on 2023/03/16.
//

import SwiftUI

struct CustomDynamicProgressView: View {
    @StateObject var progressBar: DynamicProgress = .init()
    @State var sampleProgress: CGFloat = 0
    var body: some View {
        Button("\(progressBar.isAdded ? "Stop" : "Start") Scraping") {
            if progressBar.isAdded {
                progressBar.removeProgressWithAnimations()
            } else {
                let config = ProgressConfig(title: "신한은행", progressImage: "ico_shinhan", expandedImage: "clock.badge.checkmark.fill", tint: .yellow, rotationEnabled: false)
...
struct CustomDynamicProgressView_Previews: PreviewProvider {
...
struct DynamicProgressView: View {
    ...
    func ProgressView() -> some View {
        ZStack {
            ...
            Image(uiImage: UIImage(named: config.progressImage)!)
                .resizable()
